...
function SpecChips({ payload = {} }) {
  ...
  return (
    <div className="req-summary-grid" style={{ marginTop: 10 }}>
      {chips.map((c) => (
...
export function RequestDetailsBlock({ job, compact = false }) {
  ...
  return (
    <div className="ds-section">
      ...
      <div className="req-summary" style={{ margin: 0 }}>
        <div className="req-summary-row"><span>Job ID</span><strong className="mono">{job.id}</strong></div>
        {(p.requestId || job.requestId) && (
          <div className="req-summary-row"><span>Request ID</span><strong className="mono">{p.requestId || job.requestId}</strong></div>
        )}
        <div className="req-summary-row"><span>Requested by</span><strong>{p.requestedBy || "—"}</strong></div>
        <div className="req-summary-row"><span>Type</span><strong>{(job.type || p.kind || "—").toString().toUpperCase()}</strong></div>
        ...
        {(p.templateId || p.stackId) && (
          <div className="req-summary-row">
            <span>{p.stackId ? "Stack" : "Template"}</span>
            <strong className="mono">{p.stackId || p.templateId}</strong>
          </div>
        )}
        {p.environment && <div className="req-summary-row"><span>Environment</span><strong>{p.environment}</strong></div>}
        {p.username && <div className="req-summary-row"><span>Account</span><strong className="mono">{p.username}</strong></div>}
        {when && <div className="req-summary-row"><span>Submitted</span><strong>{when}</strong></div>}
        <SpecChips payload={p} />
        {packages.length > 0 && (
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-card ds-card" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body ds-body">
          ...
          <ServiceNowPanel servicenow={job.servicenow} incident={job.incident} />
